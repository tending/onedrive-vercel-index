...
const EPUBPreview: ANY = dynamic(() => import('./previews/EPUBPreview'), {
    ssr: false,
})
...
export function ChildIcon ({child}: {child: OdFolderChildren}) {
    const {render, emoji} = renderEmoji(child.name)
    const icon: IconProp = child.file ? getFileIcon(child.name, {video: Boolean(child.video)}) : ['far', 'folder']
                ...
                {!onlyOnePage && (
                    <div className="rounded-b bg-white dark:bg-gray-900 dark:text-gray-100">
                        <div className="border-b border-gray-200 p-3 text-center font-mono text-sm text-gray-400 dark:border-gray-700">
                            - showing {size} page(s) of {isLoadingMore ? folderChildren.length : folderChildren.length} file(s) -
                        </div>
                        <button
